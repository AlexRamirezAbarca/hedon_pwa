...
import { Heart, ScanLine, Share2, Loader2 } from "lucide-react"
...
export function ConnectClient({ initialCode, initialStatus, coupleId }: ConnectClientProps) {
    ...
    return (
        <div className="w-full max-w-md space-y-8">
            {/* TAB Toggles (Visual only for now) */}
            <div className="grid grid-cols-2 gap-2 bg-zinc-900/50 p-1 rounded-xl border border-zinc-800">
                <Button variant="ghost" className="bg-zinc-800 text-white shadow-sm hover:bg-zinc-700 hover:text-white">
                    <Share2 className="w-4 h-4 mr-2" /> Mi Código
                </Button>
                <Button variant="ghost" className="text-zinc-500 hover:text-white hover:bg-zinc-800">
                    <ScanLine className="w-4 h-4 mr-2" /> Escanear
                </Button>
            </div>

            {/* QR Card */}
            <Card className="bg-zinc-900/40 border-zinc-800 p-8 flex flex-col items-center justify-center space-y-6 backdrop-blur-md">
